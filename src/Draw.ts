export type DrawProps = {
  ...
  export class Draw {
    ...
    static create(props: DrawProps): Draw | false {
      ...
      const offscreenCtx = offscreenCanvas.getContext("2d", {alpha: props.alpha})
      if (offscreenCtx === null) {
        return Draw.getCtxError(offscreenCanvas)
      }
      ctx.globalCompositeOperation = "source-over"
      offscreenCtx.globalCompositeOperation = "source-over"
      return new Draw(props.canvas, ctx, props.alpha, offscreenCanvas, offscreenCtx)
    }
    ...
    drawEmptyRectangle(x: number, y: number, width: number, height: number, color: string): void {
